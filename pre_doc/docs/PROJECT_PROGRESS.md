...
## 2026-03-07 — Baseline full-scale closeout completed (P62)

### Scope
Close out the first baseline full-scale run lineage using existing training/eval artifacts only (no retrain/restart).

### Completed
- Confirmed baseline full-scale training completion with `model_final.pth` and checkpoint lineage under `runs/wsovvis_seqformer_fullscale/fullscale_baseline_20260306_185748/1`.
- Confirmed eval-only recovery success on the same lineage under `runs/wsovvis_seqformer_fullscale_eval/fullscale_eval_recovery_p61_20260307_080800` with inference artifacts emitted.
- Established that the prior terminal failure was post-train wrapper/pipe-related (`BrokenPipeError`), not a training failure.
- Marked the baseline path as having completed a first full-scale closed loop: launch -> train -> checkpoints -> eval recovery -> artifacts.

### Status
- Baseline full-scale path is formally closed out for engineering continuity.
- Immediate decision point: move to method-scale full-train readiness (M1) or baseline writing/figure packaging (M2).

## 2026-03-06 — Stage D manuscript integration pack generated (D26)

### Scope
Convert the closed Stage D mechanism evidence into manuscript-ready assets (main text, appendix text, table/figure copy, claim-evidence mapping, and TeX placement mapping).

### Completed
- Generated `d26_manuscript_pack/` assets from existing D25 final-pack artifacts without new experiments.
- Prepared main-paper and appendix drafts with bounded claims aligned to the locked Stage D cohort findings.
- Added claim-to-evidence and TeX-structure mapping documents for Route C writeup integration.

### Status
- Stage D experiment chain remains closed.
- Immediate next priority is Route C writeup integration into `docs/WS-OVVIS.tex`.

## 2026-03-06 — Stage D final closeout completed (mechanism pack)

### Scope
Bounded Stage D mechanism closeout and manuscript-ready evidence packaging from existing synced artifacts.

### Completed
- Finalized normalized Stage D cohort pack for three pinned summaries with deterministic JSON/CSV/Markdown outputs.
- Confirmed replicated regression prevention under guard/fallback-guard (`3/3` cases).
- Confirmed no ws gain under current guard in this cohort (`mean round2 AURC delta = 0.0`).
- Completed Route A (keep-one guarded variant) negative screen (`3/3` regressions vs guarded baseline).
- Prepared Stage D paper-ready section draft, figure/table payloads, and decision memo.

### Status
- Mechanism study complete for the bounded cohort.
- Guard/fallback-guard acts as damage control in current setting.
- Route A is not promising in current form.
...
## 2026-03-06 — Stage D mechanism closeout completed (D17-D23 bounded mini-cohort evidence pack)

### Scope
Close the Stage D mechanism slice with a read-mostly evidence pack and docs sync, using existing D17-D22 artifacts only (no new training runs).

### Completed
- Produced closeout evidence-pack artifacts under:
  - `codex/2026030503_stage_c_mainline_c0_c4_semantic_slice_tier2/d23_stage_d_closeout/`
  - `cohort_summary.json`
  - `cohort_summary.csv`
  - `cohort_summary.md`
  - `cohort_plot_payload.json`
  - `STAGE_D_CLOSEOUT_SUMMARY.md`
  - `next_phase_manifest.json`
  - `next_phase_manifest.md`
- Confirmed 3/3 cases show round2 regression prevention under guard or fallback-guard:
  - mean `round2` loss delta (`guarded - unguarded`) = `-0.7494502067565918`
  - mean `round2` AURC delta (`guarded - unguarded`) = `0.0`
- Added derived per-case interpretation:
  - `damage_control_without_ws_gain=true` in all three cases.
- Locked current interpretation:
  - Stage D guard currently behaves as damage control against continued-additions regression, not as a ws-improvement driver.

### Validation
- Read-mostly aggregation/doc-sync closeout only.
- No new code-module behavior changes and no new canonical remote run required for this closeout step.

### Notes
- Source chain used for this closeout: D17, D18, D19, D20, D21, D22.
- Recommended next route: Route A first (guard refinement toward ws gain under bounded non-regression constraints).
