...
在三个summary中，unguarded分支均接受同一round2 proposal（`909002`），候选数从7增至8，并在round2达到loss `5.571925640106201`。对应地，guarded/fallback-guarded分支在同一proposal上给出reject，候选数保持7，round2 loss保持在`4.822475433349609`。这在当前cohort形成`3/3`回退阻断，mean round2 loss delta（guarded - unguarded）为`-0.7494502067565918`（Table~\ref{tab:stage_d_main_cohort}）。

就ws指标而言，round2 AURC在两种条件下均为`0.5833333333333333`，mean delta为`0.0`，因此当前证据支持damage control而非ws AURC uplift（Figure~\ref{fig:stage_d_main_round2}）。

Route A keep-one变体（`accept_top1_only_under_guard_v1`）在同一cohort下为负结果：`3/3`案例相对guarded baseline回退，且无AURC提升。主文仅保留结论，逐case细节见附录Table~\ref{tab:stage_d_appendix_per_case}与Figure~\ref{fig:stage_d_appendix_route_a}。

\begin{table}[htbp]
\centering
\caption{Stage D round2 cohort summary (n=3): guard/fallback-guard prevents regression but does not improve ws AURC.}
\label{tab:stage_d_main_cohort}
\begin{tabular}{lccc}
\hline
Cohort metric (n=3) & Unguarded & Guarded/fallback-guarded & Delta (g-u) \\
\hline
Round2 loss (mean) & 5.571925640106201 & 4.822475433349609 & -0.7494502067565918 \\
Round2 ws AURC (mean) & 0.5833333333333333 & 0.5833333333333333 & 0.0 \\
Candidate count after round2 (mean) & 8 & 7 & -1 \\
Regression prevented count & -- & 3/3 & -- \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox[c][45mm][c]{0.9\linewidth}{\centering Placeholder for Stage D main round2 comparison figure\\(source payload: d25\_stage\_d\_final\_pack/figure\_stage\_d\_round2\_compare\_payload.json)}}
\caption{Stage D round2 comparison across three pinned summaries. Unguarded accepts proposal `909002` and regresses to loss `5.571925640106201`; guarded/fallback-guarded rejects the same proposal and stays at `4.822475433349609` in `3/3` cases. Round2 AURC is unchanged (`0.5833333333333333`) across conditions.}
\label{fig:stage_d_main_round2}
\end{figure}
...
\appendix
\section*{Appendix: Stage D Mechanism Details}

Stage D链路（D17-D25）围绕continued additions的round2机制进行封闭验证，最终形成三个固定summary family（`c11b_em_t010`, `c11a_mil_t010`, `c11a_sinkhorn_t010`）的paired cohort。unguarded在三个case均接受proposal `909002`并回退到loss `5.571925640106201`；guarded/fallback-guarded在同一proposal上拒绝并保持loss `4.822475433349609`，对应`3/3`回退阻断与mean loss delta（g-u）`-0.7494502067565918`。该结论限定于当前三例cohort与该round2协议。

\begin{table}[htbp]
\centering
\caption{Per-case Stage D outcomes with Route A keep-one screen (appendix detail).}
\label{tab:stage_d_appendix_per_case}
\begin{tabular}{lcccc}
\hline
Summary ID & Unguarded loss & Guarded loss & Keep-one loss & Route A verdict \\
\hline
c11b\_em\_t010 & 5.571925640106201 & 4.822475433349609 & 5.571925640106201 & REGRESSES \\
c11a\_mil\_t010 & 5.571925640106201 & 4.822475433349609 & 5.571925640106201 & REGRESSES \\
c11a\_sinkhorn\_t010 & 5.571925640106201 & 4.822475433349609 & 5.571925640106201 & REGRESSES \\
\hline
\end{tabular}
\end{table}

Route A（`accept_top1_only_under_guard_v1`）在三个case中均重新接受round2新增候选，候选数回到8，loss回退至unguarded同值；相对guarded baseline为`3/3`负结果，且AURC均保持`0.5833333333333333`无增益。

\begin{figure}[htbp]
\centering
\fbox{\parbox[c][45mm][c]{0.9\linewidth}{\centering Placeholder for Route A keep-one appendix figure\\(per-case comparison against guarded baseline)}}
\caption{Route A keep-one screen versus guarded baseline. In all three summaries, keep-one regresses to unguarded loss (`5.571925640106201`) while guarded remains at `4.822475433349609`; AURC stays unchanged (`0.5833333333333333`).}
\label{fig:stage_d_appendix_route_a}
\end{figure}
