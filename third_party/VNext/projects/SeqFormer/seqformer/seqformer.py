...
class MaskedBackbone(nn.Module):
    ...
    def forward(self, tensor_list):
        ...
        # 仅当你的 backbone 有 patch_size（也就是 DINOv2）时才 pad
        if hasattr(self.backbone, "patch_size"):
            ps = int(self.backbone.patch_size)
            H, W = tensors.shape[-2:]
            pad_h = (ps - H % ps) % ps
            pad_w = (ps - W % ps) % ps
            if pad_h or pad_w:
                tensors = F.pad(tensors, (0, pad_w, 0, pad_h), value=0.0)
                # DETR/SeqFormer 的 mask: True 表示 padding 区域（无效）
                mask = F.pad(mask, (0, pad_w, 0, pad_h), value=True)

        xs = self.backbone(tensors)
