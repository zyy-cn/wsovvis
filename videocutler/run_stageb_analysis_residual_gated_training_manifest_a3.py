#!/usr/bin/env python3
"""
Build an A3 revised class-balanced training manifest from a residual-gated row seed pool.
...
Seed types consumed:
  ...
  - extra old-nohub-correct hard_ce_seed rows -> preservation_anchor
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--preservation_cap_per_class", type=int, default=16, help="Extra old-nohub-correct anchors selected from hard_ce_seed rows not already chosen for hard CE.")
    ...
    p.add_argument("--default_sample_weight", type=float, default=0.5, help="A3 default hard CE sample weight is intentionally lower than v1.")
    p.add_argument("--soft_sample_weight", type=float, default=1.0, help="A3 makes soft CE the main rescue branch.")
    p.add_argument("--prototype_sample_weight", type=float, default=0.10, help="A3 lowers prototype branch because v1 rescue was weak.")
    p.add_argument("--preservation_sample_weight", type=float, default=0.25)
...
def cap_for(raw_id: str, loss_family: str, args: argparse.Namespace, person_ids: set[str], hub_ids: set[str]) -> int:
    ...
    if loss_family == "preservation_anchor":
        if raw_id in person_ids:
            return max(0, args.person_preservation_cap)
        if raw_id in hub_ids:
            return max(0, args.hub_preservation_cap)
        return max(0, args.preservation_cap_per_class)
    return 0
...
def sample_weight_for(raw_id: str, loss_family: str, args: argparse.Namespace, hub_ids: set[str]) -> float:
    ...
    if loss_family == "preservation_anchor":
        return args.preservation_sample_weight
...
def select_rows(rows: List[Dict[str, str]], args: argparse.Namespace, person_ids: set[str], hub_ids: set[str]) -> Tuple[List[Dict[str, object]], List[Dict[str, object]], Dict[str, object]]:
    ...
    # A3: add preservation anchors from old-nohub-correct hard seeds that were not already selected.
    selected_keys = {(str(r.get("trajectory_id", "")), raw_norm(r.get("gt_raw_id"))) for r in selected}
    preservation_grouped: Dict[str, List[Dict[str, str]]] = defaultdict(list)
    for r in rows:
        if str(r.get("seed_type", "")).strip() != "hard_ce_seed":
            continue
        if not truth(r.get("weak_nohub_top1_is_gt")):
            continue
        key = (str(r.get("trajectory_id", "")), raw_norm(r.get("gt_raw_id")))
        if key in selected_keys:
            continue
        preservation_grouped[raw_norm(r.get("gt_raw_id"))].append(r)
    for raw_id, group in sorted(preservation_grouped.items(), key=lambda kv: kv[0]):
        if args.shuffle_ties:
            rng.shuffle(group)
        group_sorted = sorted(group, key=lambda r: make_priority(r, "hard_ce", args.prefer_multilabel))
        cap = cap_for(raw_id, "preservation_anchor", args, person_ids, hub_ids)
        take = group_sorted[:cap]
        rest = group_sorted[cap:]
        for rank, r in enumerate(take):
            out = dict(r)
            out.update({
                "manifest_use": "train",
                "loss_family": "preservation_anchor",
                "selection_rank_within_class_loss": rank + 1,
                "class_cap_used": cap,
                "is_person_raw_id": str(raw_id in person_ids),
                "is_hub_raw_id": str(raw_id in hub_ids),
                "sample_weight": sample_weight_for(raw_id, "preservation_anchor", args, hub_ids),
                "selection_reason": "selected_by_preservation_anchor_cap",
            })
            selected.append(out)
            selected_keys.add((str(r.get("trajectory_id", "")), raw_norm(r.get("gt_raw_id"))))
        for r in rest:
            rr = dict(r)
            rr.update({
                "reject_reason": "over_class_cap_preservation_anchor",
                "loss_family": "preservation_anchor",
                "class_cap_used": cap,
            })
            rejected.append(rr)
    ...
    # Stable manifest order: hard CE first, soft, prototype, then preservation anchors.
    loss_order = {"hard_ce": 0, "soft_ce": 1, "prototype_calibration": 2, "preservation_anchor": 3}
...
def summarize(rows: List[Dict[str, object]], by_class_rows: List[Dict[str, object]], rejected_rows: List[Dict[str, object]], args: argparse.Namespace, person_ids: set[str], hub_ids: set[str]) -> Dict[str, object]:
    ...
    return {
        ...
        "interpretation": {
            ...
            "preservation_anchor": "Old-nohub-correct hard seeds not already selected, used only to reduce forgetting.",
...
def main() -> None:
    ...
    md += [
        ...
        "- A3 adds preservation_anchor rows from old-nohub-correct hard seeds not already selected.",
    ]
