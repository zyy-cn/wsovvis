#!/usr/bin/env python3
"""
Build an A5 soft-preserve-lock training manifest from a residual-gated row seed pool.

A5 is designed after A4 showed strong soft rescue but remaining old-nohub-correct forgetting:
...
This planner is read-only. It writes lightweight CSV/JSON/MD artifacts only. A5 keeps the A4 family structure but selects a stronger preservation-anchor pool and assigns larger preservation sample weights.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--preservation_cap_per_class", type=int, default=48)
    ...
    p.add_argument("--preservation_sample_weight", type=float, default=2.0)
    p.add_argument("--hub_sample_weight", type=float, default=1.0)
...
def ensure_path_args(args: argparse.Namespace) -> Tuple[Path, Path]:
    ...
    if args.out_dir:
        ...
        out_dir = Path(args.run_root) / "analysis" / "residual_gated_training_manifest_a5" / args.dataset_name
        ...
        for raw_id, group in sorted(grouped.items(), key=lambda kv: kv[0]):
            ...
            for rank, r in enumerate(take):
                ...
                out.update({
                    ...
                    "selection_reason": f"a5_selected_by_{loss_family}_cap",
                })
                selected.append(out)
            for r in rest:
                rr = dict(r)
...
def summarize(selected: List[Dict[str, object]], by_class_rows: List[Dict[str, object]], rejected: List[Dict[str, object]], args: argparse.Namespace, person_ids: set[str], hub_ids: set[str]) -> Dict[str, object]:
    ...
    return {
        ...
        "args": vars(args),
        "interpretation": {
            "a5_policy": "Soft CE remains the positive-set rescue loss. Hard seed rows are used only as stronger preservation anchors with larger sample weights / margin-lock training. Prototype rows are excluded from training.",
            "gpu_required": False,
...
def main() -> None:
    ...
    md = [
        "# Residual-Gated A5 Soft-Preserve-Lock Manifest",
        "",
    ...
    md += [
        ...
        "",
        "## Notes",
        "- A5 selects soft_ce + stronger preservation_anchor only.",
    ...
    (out_dir / "RESIDUAL_GATED_A5_MANIFEST_TAKEOVER.md").write_text("\n".join(md), encoding="utf-8")
