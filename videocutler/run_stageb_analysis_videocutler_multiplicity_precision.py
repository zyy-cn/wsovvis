...
def _normalise_rle(rle: Any, height: Optional[int] = None, width: Optional[int] = None) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(rle, dict):
        # Standard COCO RLE: {'size':[h,w], 'counts': ...}
        if 'counts' in rle and 'size' in rle:
            return rle
        # Some records wrap masks.
...
def _extract_mask_sequence(obj: Dict[str, Any]) -> List[Optional[Dict[str, Any]]]:
    ...
    seq = _get_first(obj, [
        ...
        'frame_masks', 'pred_masks', 'trajectory_masks', 'mask_rles'
    ])
...
def tube_iou(seq1: Sequence[Optional[Dict[str, Any]]], seq2: Sequence[Optional[Dict[str, Any]]]) -> float:
    ...
    for i in range(n):
        ...
        aa = _area(a)
        bb = _area(b)
        ii = _intersection(a, b)
        inter += ii
        union += max(0.0, aa + bb - ii)
...
def main() -> int:
    ...
    for vid in videos:
        ...
        for cls in gt_classes | yprime:
            ...
            for tr in trjs:
                best_for_tr_cls = 0.0
                for g in gts_cls:
                    best_for_tr_cls = max(best_for_tr_cls, tube_iou(g['masks'], tr['masks']))
    ...
    summary: Dict[str, Any] = {
        'status': 'PASS' if PYCOCOTOOLS_AVAILABLE and exact_iou_pairs > 0 else 'INVALID_INPUT_SURFACE',
    ...
    if summary['status'] == 'PASS':
        ...
        summary['interpretation'] = {'verdict': 'invalid_input_surface_no_exact_iou_pairs'}
