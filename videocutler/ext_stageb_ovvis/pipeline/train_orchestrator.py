    ...
    if plan.pipeline == 'legacy':
        ...
        sink = run_reservoir_sinkhorn_no_unknown(
            ...
            config=ReservoirSinkhornNoUnknownConfig(
                ...
                sinkhorn_enable_positive_protection=bool(getattr(plan, 'sinkhorn_enable_positive_protection', False)),
                sinkhorn_positive_margin_threshold=float(getattr(plan, 'sinkhorn_positive_margin_threshold', 0.15)),
                sinkhorn_positive_margin_temp=max(1.0e-6, float(getattr(plan, 'sinkhorn_positive_margin_temp', 0.10))),
                sinkhorn_positive_null_cap=float(getattr(plan, 'sinkhorn_positive_null_cap', 0.40)),
                sinkhorn_positive_redistribute_mode=str(getattr(plan, 'sinkhorn_positive_redistribute_mode', 'best_y')),
                residual_peeling_mode=str(getattr(plan, 'residual_peeling_mode', 'off')),
                residual_schedule_csv=str(getattr(plan, 'residual_schedule_csv', '') or ''),
                residual_variant=str(getattr(plan, 'residual_variant', 'person_aware')),
                residual_annotation_json=str(getattr(plan, 'residual_annotation_json', '') or ''),
                residual_split_json=str(getattr(plan, 'residual_split_json', '') or ''),
                residual_round_epoch_plan=str(getattr(plan, 'residual_round_epoch_plan', '5,5,3,2')),
                residual_candidate_policy=str(getattr(plan, 'residual_candidate_policy', 'base_residual')),
                known_null_gate=str(getattr(plan, 'known_null_gate', 'off')),
                known_null_margin=float(getattr(plan, 'known_null_margin', 0.10)),
