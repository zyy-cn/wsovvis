#!/usr/bin/env python3
"""A8 P525 prototype-supervised projector audit.

This is a read-only diagnostic: fit a text->DINO linear projector directly from
train-visible class visual prototypes, then evaluate row-level GT ranking on
train/val GT trajectories and val full-vocabulary base/novel splits.

It does not use weak labels, clip-level Hungarian assignment, row-level GT as
training samples, or any A8 checkpoint. The only supervision is one visual class
prototype per train-visible class.
"""
...
def _fit_map(x: np.ndarray, y: np.ndarray, method: str, alpha: float) -> np.ndarray:
    ...
    if method == "ridge":
        # Dual ridge: W = X^T (X X^T + alpha I)^-1 Y. More stable for 4096-d text.
        n = int(x.shape[0])
...
def _eval_proto(projected_all: np.ndarray, ids_all: Sequence[int], visual_proto: Mapping[int, np.ndarray], eval_ids: Sequence[int], candidate_ids: Sequence[int]) -> Dict[str, Any]:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="P525 prototype-supervised projector audit")
    ...
    for variant in variants:
        ...
        fit_ids = [rid for rid in visible if rid in text_id_set and rid in train_proto]
        if len(fit_ids) < 2:
            raise RuntimeError(f"not enough fit ids for {variant}: {len(fit_ids)}")
        x_fit = _submatrix(text_ids, text_mat, fit_ids)
        y_fit = np.stack([train_proto[rid] for rid in fit_ids], axis=0).astype(np.float32)
        ...
        for method in methods:
            ...
            np.savez_compressed(run_dir / "prototype_supervised_mapper.npz", W=w.astype(np.float32), fit_raw_ids=np.asarray(fit_ids, dtype=np.int64))
            ...
            proto_train = _eval_proto(proj_all, text_ids, train_proto, fit_ids, candidate_visible_ids)
            proto_val = _eval_proto(proj_all, text_ids, val_proto, [rid for rid in visible if rid in val_proto], candidate_visible_ids)
            ...
            _write_json(run_dir / "summary.json", {
                "status": "PASS",
                "name": run_name,
                "definition": "P525 prototype-supervised text-to-DINO projector: fit one text anchor to one train visual class prototype for each visible class.",
                "variant": variant,
                "method": method,
                ...
                "class_proto_train": proto_train,
                "class_proto_val": proto_val,
                "artifacts": {
                    "mapper_npz": str(run_dir / "prototype_supervised_mapper.npz"),
                ...
                "policy": {
                    ...
                    "uses_class_prototype_gt_for_training": True,
                    "fit_scope": "train_visible_525_class_prototypes",
                },
            })
    ...
    _write_csv(output_root / "p525_proto_supervised_summary.csv", summary_rows)
    report = {
        "status": "PASS",
        "definition": "Full P525 prototype-supervised projector audit. Direct class-prototype supervision; no weak supervision; row-level train/val ranking and val full-vocab base/novel ranking.",
        ...
        "val_meta": val_meta,
        "artifacts": {"summary_csv": str(output_root / "p525_proto_supervised_summary.csv")},
        "rows": run_reports,
    }
    _write_json(output_root / "p525_proto_supervised_report.json", report)
