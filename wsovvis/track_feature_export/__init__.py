"""Stage B track feature export v1 producer/validator utilities."""

from .bridge_from_stageb_v1 import (
    build_track_feature_export_v1_from_stageb_bridge_input,
    build_track_feature_export_v1_from_stageb_bridge_input_json,
    convert_stageb_bridge_input_to_task_input_v1,
    load_stageb_bridge_input,
)
from .feature_export_enablement_v1 import (
    build_feature_export_enablement_v1,
    load_feature_export_enablement_input,
)
from .real_run_feature_export import export_feature_enablement_from_real_run
from .real_stageb_sidecar_bridge_input_builder_v1 import (
    build_normalized_bridge_input_from_real_stageb_sidecar,
    build_normalized_bridge_input_from_real_stageb_sidecar_to_json,
)
from .stagec_loader_v1 import (
    StageCExportLoadError,
    StageCExportSplitView,
    StageCTrackMetadata,
    StageCTrackRecord,
    StageCVideoRecord,
    load_stageb_export_split_v1,
)
from .stagec1_attribution_mil_v1 import (
    StageC1AttributionError,
    StageC1DecoderConfig,
    StageC1EmConfig,
    StageC1SinkhornC43Config,
    StageC1SinkhornConfig,
    StageCLabelPrototypeInventory,
    StageC1MilConfig,
    StageC1MilResult,
    StageC1TrackScoreRecord,
    compute_stagec1_em_v1_scores,
    compute_stagec1_sinkhorn_v1_scores,
    compute_stagec1_mil_baseline_scores,
    compute_stagec1_labelset_proto_baseline_scores,
    load_stagec_label_prototype_inventory_v1,
    load_stagec_labelset_lookup,
    run_stagec1_mil_baseline_offline,
    write_stagec1_mil_artifacts,
)
from .stagec_clip_text_prototype_cache_v1 import (
    StageCClipTextPrototypeCacheEntryV1,
    build_stagec_clip_text_prototypes_v1,
    compute_stagec_clip_text_cache_key_v1,
    get_or_build_stagec_clip_text_prototype_cache_v1,
    load_stagec_clip_text_prototype_cache_v1,
    save_stagec_clip_text_prototype_cache_v1,
)
from .stagec_semantic_slice_v1 import (
    STAGEC_BG_LABEL_ID,
    STAGEC_UNK_FG_LABEL_ID,
    StageCCandidateSetV1,
    StageCSemanticAssignmentOutputV1,
    StageCSemanticBatchV1,
    StageCSemanticLossHookInputV1,
    StageCSemanticLossHookOutputV1,
    StageCSemanticSliceError,
    build_stagec_candidate_matrix_from_track_label_sets_v1,
    build_stagec_candidate_set_v1,
    build_stagec_prototype_candidate_stub_v1,
    compute_stagec_semantic_loss_hook_stub_v1,
    merge_stagec_candidate_label_ids_v1,
    normalize_stagec_semantic_batch_v1,
    run_stagec_assignment_stub_v1,
    select_stagec_topk_label_ids_from_scores_v1,
)
from .v1_core import (
    ExportContractError,
    build_track_feature_export_v1,
    load_task_local_input,
    validate_track_feature_export_v1,
)

__all__ = [
    "ExportContractError",
    "build_feature_export_enablement_v1",
    "build_track_feature_export_v1",
    "build_track_feature_export_v1_from_stageb_bridge_input",
    "build_track_feature_export_v1_from_stageb_bridge_input_json",
    "build_normalized_bridge_input_from_real_stageb_sidecar",
    "build_normalized_bridge_input_from_real_stageb_sidecar_to_json",
    "StageCExportLoadError",
    "StageCExportSplitView",
    "StageCTrackMetadata",
    "StageCTrackRecord",
    "StageCVideoRecord",
    "StageC1AttributionError",
    "StageC1DecoderConfig",
    "StageC1EmConfig",
    "StageC1SinkhornC43Config",
    "StageC1SinkhornConfig",
    "StageCLabelPrototypeInventory",
    "StageC1MilConfig",
    "StageC1MilResult",
    "StageC1TrackScoreRecord",
    "StageCSemanticAssignmentOutputV1",
    "StageCCandidateSetV1",
    "StageCSemanticBatchV1",
    "StageCClipTextPrototypeCacheEntryV1",
    "StageCSemanticLossHookInputV1",
    "StageCSemanticLossHookOutputV1",
    "StageCSemanticSliceError",
    "STAGEC_BG_LABEL_ID",
    "STAGEC_UNK_FG_LABEL_ID",
    "build_stagec_candidate_matrix_from_track_label_sets_v1",
    "build_stagec_candidate_set_v1",
    "build_stagec_clip_text_prototypes_v1",
    "compute_stagec1_em_v1_scores",
    "compute_stagec1_sinkhorn_v1_scores",
    "compute_stagec1_mil_baseline_scores",
    "compute_stagec1_labelset_proto_baseline_scores",
    "compute_stagec_clip_text_cache_key_v1",
    "compute_stagec_semantic_loss_hook_stub_v1",
    "convert_stageb_bridge_input_to_task_input_v1",
    "export_feature_enablement_from_real_run",
    "get_or_build_stagec_clip_text_prototype_cache_v1",
    "build_stagec_prototype_candidate_stub_v1",
    "load_stagec_label_prototype_inventory_v1",
    "load_stagec_labelset_lookup",
    "load_stagec_clip_text_prototype_cache_v1",
    "load_feature_export_enablement_input",
    "load_stageb_export_split_v1",
    "load_stageb_bridge_input",
    "load_task_local_input",
    "merge_stagec_candidate_label_ids_v1",
    "normalize_stagec_semantic_batch_v1",
    "run_stagec1_mil_baseline_offline",
    "run_stagec_assignment_stub_v1",
    "save_stagec_clip_text_prototype_cache_v1",
    "select_stagec_topk_label_ids_from_scores_v1",
    "validate_track_feature_export_v1",
    "write_stagec1_mil_artifacts",
]
